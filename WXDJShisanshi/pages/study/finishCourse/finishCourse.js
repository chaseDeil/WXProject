...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    util.requestGet('learnManagementAction!finishedCourse.action', { userId: app.globalData.userId, page: that.data.page, pageSize: 20 }, function (res) {
      that.setData({
  ...
  loadmore:function() {
    ...
    util.requestGet('learnManagementAction!finishedCourse.action', { userId: app.globalData.userId, page: page, pageSize: 20 }, function (res) {
      wx.hideLoading()
      if (res.data['objList'].length == 0) {
        wx.showToast({
          title: '没有更多数据',
